...
def test_remove_background_handles_alpha_margin_and_feathering():
    image = Image.new("RGBA", (24, 24), (0, 0, 0, 0))
    image.paste((255, 255, 255, 255), (4, 4, 20, 20))
    image.paste((220, 30, 30, 255), (8, 8, 16, 16))

    result = cardglow.remove_background(
        image, tolerance=1, feather=1, corner_sample=2
    )

    assert result.getpixel((0, 0))[3] == 0
    assert result.getpixel((6, 10))[3] < 255
...
@pytest.mark.parametrize("draw_grid,draw_vignette", [(False, False), (True, True)])
def test_make_card_returns_opaque_rgb_image(draw_grid, draw_vignette):
